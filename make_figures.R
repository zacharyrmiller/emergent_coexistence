...
show_n <- c(6, 8, 10) # which values of n should be plotted
...
pa <- null_distribution %>% 
  filter(n %in% show_n) %>%
  ggplot() + 
  aes(x = null_directed_triangles / null_reps) +
  ...
  expand_limits(x = 0) + 
  xlab("Directed triangles") + ylab("Density") + 
  theme_classic() + 
...
pb <- results %>% group_by(n, exclusion_frac) %>% 
  ...
  facet_wrap(.~n, scales = "free", nrow = 1) + 
  xlab("Exclusion fraction") + ylab("Directed triangles") + 
  theme_classic() + 
  theme(legend.position = "none", 
